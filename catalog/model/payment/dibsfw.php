...
class ModelPaymentDibsfw extends Model {
    ...
    const MODULE_VERSION = 'opc_fw_vqm_3.0.1';
	...
	$method_data = array();
        ...
        $titleImg = "<img src=\"{$this->config->get('config_url')}catalog/view/theme/default/image/dibsf.png\"width=\"178\"height=\"60\"/>"; 
        if ($status) {  
            $method_data = array( 
                'code'       => 'dibsfw',
                'title'      => $titleImg,
    ...
    private function collectStandartParams($order) {
        ...
        $order['yourRef'] =  $requestParams['orderid'];
    ...
    private function api_dibs_hextostr($sHMAC) {
        $sRes = '';
    ...
    private function collectInvoiceParams($order) {
        ...
        $root->appendChild($doc->createElement("yourRef"))
             ->appendChild($doc->createTextNode($order['yourRef']));
        $i = 1;
